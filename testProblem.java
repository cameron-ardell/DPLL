...
public class testProblem {
    ...
    */
    ...
    //array that holds how many variables are active in each block
    private int[] activeVarInBlock;
    /*keeps track of which block of variables is currentely being paid attention
    to. Use this index to look up first variable in blockFirstIndex*/
 ...
 //read file with problem into code       
    ...
    public void testValues() {
        ...
        //System.out.println("activeVar: " + activeVariables);
...
//        System.out.println();
    ...
    public double solve(int curVarIndex) {
        ...
        printArray();
      printActiveClauses();
       System.out.println("block index: " + curBlockIndex);
        //printBlockFirstIndexArray();
        printActiveVarInBlockArray();
...
/*            System.out.println("advance backwards");
        ...
        if(newVarCol == -1){
            ...
            if(newVarCol == -1){
                //if(splitType != "naive") {printActiveClauses();}
                newVarCol = split();
                 System.out.print("split heuristic, ");
            }
            else {
    ...
    public int update(int literal){
        ...
        //means that literal appears in no active clauses
        //but should still update counters since other variables could be negated later
        /*if((allData[activePosLitRow][litCol] == 0  && allData[activeNegLitRow][litCol] == 0)){
            return;
        }*/
        ...
        for(int i = 0; i < numClauses; i++) {
         //   System.out.println("i = " + i);
           // System.out.print("running case: ");
           
           /*boolean yes = false;
           if (checkUnitClause(i) == true) {
               numberOfUnitClauses += 1;
               System.out.println(i);
               yes = true;
               //System.out.println(numberOfUnitClauses + " " + i);
           }
                     */
            //case 1
            ...
            //case 3
               ...
               // System.out.print("3\n");
                ...
                //if was a unit clause and is now satisfied, don't count it anymore
               // System.out.println("satisfying stuff");
                //if(yes) { numberOfUnitClauses -= 1; }
    ...
    public int split(){
        ...
        switch(splitType) {
           ...
           case "MAC":
               ...
               varIndex = numActiveClausesSplit();
               break;
           case "MSAC":
               varIndex = numSignedActiveClausesSplit();
               break;
            default:
    ...
    public int numSignedActiveClausesSplit() {
        
         //get section of variables to work with
        int beginInd = blockFirstIndex[curBlockIndex];
        int endInd = blockFirstIndex[curBlockIndex + 1];
    ...
    public int numActiveClausesSplit() {
    ...
    public void makeBlockFirstIndexArray(){
        ...
        activeVarInBlock = new int[numBlocks];
        
        
        for (int i = 0; i < numBlocks; i++) {
            
            activeVarInBlock[i] = blockFirstIndex[i + 1] - blockFirstIndex[i];
    ...
    public void printActiveVarInBlockArray() {
        for (int i = 0; i < activeVarInBlock.length; i++){
            System.out.print(activeVarInBlock[i] + " ");
        }
        System.out.println("");
    }
    ...
    public void updateBlockIndex() {
        ...
        //keep track of current block index code is at
        int curVarBlockInd = 0;
        activeVarInBlock[0] = 0;
        
        //first block to have active variables will be block index
        //setting as maximum value so will be reduced
        //if there are no active variables, then index won't matter
        int nextCurBlockIndex = numBlocks;
        
        //recount amount of active variables
        for (int i = 0; i < numVariables; i++) {
            
            int cur = (int)allData[blockIndexRow][i];
            int resolved = (int)allData[assignedBoolRow][i];
            
            //if current block index changes, set next one to zero and update
            //current index
            if(cur != curVarBlockInd) {
                activeVarInBlock[cur] = 0;
                curVarBlockInd = cur;
            }
            
            //if bool is not resolved, then is active, account for it
            if (resolved == 0) {
                activeVarInBlock[curVarBlockInd] += 1;
                
                //this means there is at least one active variable in this block
                //check if it is lowest block with active variables
                if(nextCurBlockIndex > curVarBlockInd) {
                    nextCurBlockIndex = curVarBlockInd;
                }
            }
            //otherwise ignore it
            else {
        ...
        //instead just go to first block that has active variables
        //can do in above for loop
        //fuck everything below this
        //profit
        
        
/*
        //if enough satisfied variables to go to next block type
        if( 0 == activeVarInBlock[curBlockIndex] && sat_var != numVariables
                && curBlockIndex != numBlocks -1){
            //System.out.println(curBlockIndex);
            curBlockIndex += 1;
        }
        //if all of the variables in the current block are already unsatisfied
        //and any of the ones below it do not, decrease block index to that
        else if(curBlockIndex > 0 &&
                activeVarInBlock[curBlockIndex] ==
                blockFirstIndex[curBlockIndex + 1] - blockFirstIndex[curBlockIndex] ) {
            
            for(int i = curBlockIndex; i > -1; i--){
                
                if(activeVarInBlock[i] > 0){
                    curBlockIndex = i;
                    break;
                }
    ...
    public int[] pureVarSplit() {
        int index = -1;
        int purity = 0;
        
        //not a choice variable, or no pure variables allowed, can't find
        //pure variable then
        if((allData[total_row][blockFirstIndex[curBlockIndex]] != -1) ||
                pureVariables == false) {
